...
# Amplitude MCP 직접 조회 (파일 불필요)
/analyst --source amplitude --type [분석유형] --topic "[분석 주제]"
/analyst --source amplitude --type funnel --topic "[주제]" --initiative TM-XXXX
```

### `--source amplitude` 모드

파일 없이 **Amplitude MCP를 통해 직접 데이터를 쿼리**한다.
Amplitude MCP가 연결된 경우에만 사용 가능 (`claude mcp add -t http -s user Amplitude ...`).

| 분석 유형 | Amplitude에서 자동 수집하는 데이터 |
|---------|--------------------------------|
| `trend` | 지정 이벤트 또는 DAU/WAU/MAU 시계열 |
| `funnel` | 지정 이벤트 시퀀스의 단계별 전환율 |
| `cohort` | 신규 유저 또는 특정 코호트의 리텐션 |
| `ab-test` | 실험 결과 (control vs variant 지표 비교) |
| `segment` | 세그먼트/프로퍼티별 이벤트 분포 |
| `diagnose` | 이상 지표 관련 이벤트 추이 자동 수집 |

Amplitude 프로젝트명이나 이벤트명을 `--topic`에 명시하면 더 정확하게 쿼리한다.
예: `--topic "무신사앱 홈→상품→결제 퍼널"`, `--topic "앱푸시 수신 후 구매 전환"`
...
# --- Amplitude MCP 직접 조회 ---
# 파일 없이 Amplitude에서 바로 데이터 수집 후 분석

# Amplitude 퍼널 분석
/analyst --source amplitude --type funnel --topic "홈→상품→결제 전환 퍼널"

# Amplitude DAU 트렌드
/analyst --source amplitude --type trend --topic "최근 4주 DAU 트렌드"

# Amplitude A/B 테스트 결과
/analyst --source amplitude --type ab-test --topic "앱푸시 메시지 실험 결과"

# Amplitude 지표 이상 원인 진단
/analyst --source amplitude --type diagnose --topic "3월 4주차 구매전환율 급락 원인" --initiative TM-2061
```
