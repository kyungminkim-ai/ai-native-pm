...
6. **[레지스트리 등록]** 파일 저장 직후 레지스트리에 등록한다:
   ```bash
   python3 scripts/registry.py register --type redteam --path prd-agent-system/output/redteam_{YYYYMMDD}_{주제}.md --topic "{주제}"
   ```
7. 완료 후 요약(질문 수, Critical/Important/Minor 분포, 상위 3개 Critical 질문, scope 결과 요약)을 출력한다.
