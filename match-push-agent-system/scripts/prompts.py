...
다음 소재 정보를 바탕으로 희소성/긴급성 강조형(V3 SCARCITY) 앱푸시 본문을 작성하세요.
...
## 작성 규칙
1. 반드시 `(광고) `로 시작
2. 길이: 30~50자 (수신거부 문구 제외)
3. 희소성·긴급성을 핵심 소구로 활용 ("한정수량", "마감 임박", "선착순", "오늘까지", "D-day" 등)
4. 수치 나열 금지 — 대신 긴박감·기회 강조
5. 명사형 종결 권장, 짧고 임팩트 있게
...
## 좋은 예시
- `(광고) 오늘 자정 마감, 마지막 수량`
- `(광고) 선착순 한정 증정, 지금이 마지막 기회`
- `(광고) 품절 전 마지막 찬스, 한정 수량 소진 중`
...
def build_review_prompt(
    ...
    v3_line = f"\n- 본문 V3 (희소성/긴급성): {v3_clean or '(없음)'}" if v3_clean else ""
