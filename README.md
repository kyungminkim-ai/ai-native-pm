...
PM 입력 (슬래시 명령어 or 자연어)
         ...
         │    PRD 파일 or 이니셔티브 키                               │
         │                                                          │
         │  ICP 정의 → 채널 전략 → 런치 타임라인 → 성공 지표          │
         │    └─▶ output/GTM_Brief_{날짜}_{주제}.md ──────────────────┘
         │
         ├──/report─────────────────────────────────────────────────┐
         │                                                          │
...
| 커맨드 | 핵심 기능 |
|--------|----------|
| `/staff` | 비서실장 — 모든 요청 수신·위임·조율 |
| `/discovery` | 시장 탐색 + 가상 인터뷰 + 디스커버리 보고서 |
| `/prd` | Rough Note → PRD + Red Team + Confluence 업로드 |
| `/red` | PRD 단독 Red Team 검증 |
| `/two-pager` | 경영진용 2-Pager 의사결정 문서 |
| `/epic` | PRD → Jira 에픽 티켓 자동 생성 |
| `/jira` | 자연어 → Jira 티켓 일괄 생성 (컨펌 후 실행) |
| `/task-ticket` | 대화 → MEMB Task 티켓 생성 |
| `/gtm` | PRD → GTM 브리프 생성 (ICP·채널 전략·성공 지표) |
| `/report` | C레벨 보고서 품질 검토 + 종합 판정 |
| `/pgm` | Jira + 메모 → 주간 보고서 + 회의록 + Slack 요약 |
| `/mail` | Confluence / MD → Gmail 발송 |
| `/data` | CRM·Amplitude·Databricks·파일 → 조회+분석+차트 |
| `/crm-analysis` | CRM 전광판 지표 조회 · 트렌드 · 채널 비교 분석 |
| `/databricks` | Databricks 탐색·쿼리 실행·분석 |
| `/analyst` | 트렌드·세그먼트·퍼널·코호트·A-B 분석 |
| `/chart` | CSV → Chart.js 인터랙티브 HTML 차트 |
| `/figma` | Figma 화면 분석·설계서·PRD 초안 생성 |
| `/discovery` | 시장·사용자·경쟁사 리서치 및 Discovery 분석 |
| `/market-research` | 페르소나·경쟁사 포지셔닝·고객 여정 지도 |
| `/okr` | OKR 초안 수립 + 이니셔티브 정합성 검토 |
| `/meeting` | 회의록 작성 + Confluence 업로드 + Calendar 등록 |
| `/kb` | 마크다운 문서를 input/kb/에 저장·인덱싱·쿼리 |
| `/slack` | Slack 채널 조회·검색·메시지 발송 |
| `/strategy` | 전략적 고민 자문 (CEP·AI 개발·빌더 마인드셋) |
| `/chronicle` | 업무 이력 wiki 관리 + 주간/월간 회고·포트폴리오 |
| `/ticket-review` | CSV/JQL 기반 Jira 과제 검토·클러스터링 |
| `/archive` | 30일 이상 산출물 → archive/ 정리 |
...
```
pm-studio/
├── CLAUDE.md                        ← 비서실장 오케스트레이터
├── CAPABILITY_MAP.md                ← 팀 역량 상세 정의
├── WORKFLOW_PATTERNS.md             ← 복합 워크플로우 패턴
├── CONVENTIONS.md                   ← 공통 규약
├── README.md                        ← 이 파일
├── input/                           ← 외부 입력 자료 (gitignore)
│   ├── initiatives/                 ← 이니셔티브 컨텍스트
│   │   ├── index.md                 ← 분기별 이니셔티브 현황 인덱스
│   │   ├── _template/               ← 이니셔티브 추가용 템플릿
│   │   ├── _kb/                     ← 지식 베이스
│   │   ├── 2026Q1/
│   │   └── 2026Q2/
│   ├── kb/                          ← 로컬 Knowledge Base (/kb 스킬)
│   ├── data/                        ← 분석용 CSV/파일
│   ├── meetings/                    ← 회의 관련 자료
│   └── refs/                        ← 참조 문서
...
├── scripts/                         ← 유틸리티 스크립트
├── .claude/
│   └── skills/                      ← 33개 스킬 (비서실장 위임 대상)
│       ├── staff/                   ← /staff — 비서실장
│       ├── prd/                     ← /prd — PRD 작성
│       ├── red/                     ← /red — Red Team 검증
│       ├── two-pager/               ← /two-pager — 2-Pager 작성
│       ├── epic/                    ← /epic — 에픽 분해 + Jira 생성
│       ├── jira/                    ← /jira — 티켓 일괄 생성
│       ├── task-ticket/             ← /task-ticket — MEMB Task 생성
│       ├── gtm/                     ← /gtm — GTM 브리프
│       ├── report/                  ← /report — 보고서 품질 검토
│       ├── pgm/                     ← /pgm — Weekly Flash Report
│       ├── mail/                    ← /mail — Gmail 발송
│       ├── data/                    ← /data — 통합 데이터 분석
│       ├── crm-analysis/            ← /crm-analysis — CRM 전광판
│       ├── databricks/              ← /databricks — SQL 탐색·분석
│       ├── analyst/                 ← /analyst — 데이터 분석 인사이트
│       ├── chart/                   ← /chart — CSV → Chart.js HTML
│       ├── figma/                   ← /figma — Figma 화면 분석
│       ├── discovery/               ← /discovery — 시장·사용자 리서치
│       ├── market-research/         ← /market-research — 페르소나·경쟁사
│       ├── okr/                     ← /okr — OKR 수립·검토
│       ├── meeting/                 ← /meeting — 회의록 + Calendar
│       ├── kb/                      ← /kb — 로컬 Knowledge Base
│       ├── slack/                   ← /slack — Slack 조회·발송
│       ├── strategy/                ← /strategy — 전략 자문
│       ├── chronicle/               ← /chronicle — 커리어 성장 wiki
│       ├── ticket-review/           ← /ticket-review — Jira 과제 검토
│       ├── archive/                 ← /archive — 산출물 정리
│       ├── push-campaign/           ← /push-campaign — 앱푸시 캠페인
│       ├── confluence-tool/         ← Confluence API 공통 (내부)
│       ├── gmail-tool/              ← Gmail 발송 공통 (내부)
│       ├── notion-tool/             ← Notion API 공통 (내부)
│       └── work-log/                ← /work-log — 주간 작업 로그
...
│   └── .claude/agents/
│       └── report-red-team/
├── pgm-agent-system/                ← Weekly Flash Report 시스템
│   ├── CLAUDE.md
│   ├── input/
│   └── output/
├── two-pager-agent-system/          ← 2-Pager 작성 시스템
│   └── output/
├── data-agent-system/               ← 통합 데이터 분석 시스템
│   ├── CLAUDE.md
│   ├── input/
│   └── output/
├── crm-analysis-agent-system/       ← CRM 전광판 분석 시스템
│   ├── CLAUDE.md
│   └── output/
├── analyst-agent-system/            ← 데이터 분석 인사이트 시스템
│   ├── CLAUDE.md
│   ├── input/
│   └── output/
├── databricks-agent-system/         ← Databricks SQL 탐색·분석 시스템
│   ├── CLAUDE.md
│   └── output/
├── figma-agent-system/              ← Figma 화면 분석·설계서 시스템
│   ├── CLAUDE.md
│   └── output/
├── chronicle/                       ← 커리어 성장 wiki (LLM Wiki)
│   ├── input/
│   └── output/
└── discovery-intelligence-system/  ← Product Discovery Intelligence System
    ├── CLAUDE.md
    ├── scripts/
    │   ├── fetch_amplitude.py
    │   ├── fetch_braze.py
...
[Phase 1] epic-architect
    ...
    ↓ 사용자 컨펌 ("실행" 입력)
[Phase 2] Jira 티켓 일괄 생성
    → 에픽 티켓 N개 (라벨 상속 + 직무 라벨 추가)
    → Jira 티켓 URL 목록 출력
```
...
### 에이전트 시스템 6: C레벨 Report Quality System
...
| `two-pager-agent-system/output/two_pager_*.md` | 생성된 2-Pager 문서 |
...
| `data-agent-system/output/` | 통합 데이터 분석 결과 |
| `crm-analysis-agent-system/output/` | CRM 전광판 분석 결과 |
| `analyst-agent-system/output/` | 데이터 분석 인사이트 보고서 |
| `chronicle/output/` | 커리어 성장 wiki 산출물 |
...
| `input/kb/` | 로컬 Knowledge Base 문서 (/kb 스킬) |
...
> **최종 업데이트**: 2026-05-04 | **스킬**: 33개 | **구동 환경**: Claude Code (claude-sonnet-4-6) + Atlassian Confluence REST API + Jira REST API v3 + Google Workspace API + Gmail SMTP + Amplitude MCP + Databricks MCP + Slack MCP + Figma MCP
