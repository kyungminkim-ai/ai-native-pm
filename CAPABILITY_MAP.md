...
```
pm-studio
├── [기획팀]        PRD 작성 · Red Team 검증 · 2-Pager · GTM 플래닝
├── [PGM팀]        Weekly Flash · C레벨 보고서 검토 · OKR 수립
├── [디스커버리팀]  시장/제품 Discovery 분석 · 시장 리서치 (페르소나/경쟁사/여정 지도)
...
└── [커리어팀]      주간/월간 업무 결산 · 피드백 · 플래닝 · 포트폴리오 (Chronicle)
```
...
| **GTM 플래닝** | Skill: `/gtm` | claude-sonnet-4-6 |
...
**GTM, 고투마켓, 런치 전략, ICP, 채널 전략, Go/No-Go, 런치 체크리스트, 성공 지표, 포지셔닝**
...
[기획 단계]      /prd → (Phase 4: Red Team 자동) → (Phase 5: 보강 자동 → _v2.md)
                              ...
                              → qa-agent
[런치 준비]      /gtm {PRD 파일 경로} → GTM 체크리스트 + 채널 전략 + Go/No-Go 기준
```
...
#### GTM 플래닝 (`/gtm`)
- **입력**: 기능명 또는 `--prd {경로}` + 선택적 `--initiative TM-XXXX`
- **출력**: `output/gtm/gtm_{YYYYMMDD}_{주제}.md`
- **단계**: 포지셔닝(ICP+UVP) → 채널 전략 → 런치 타임라인 → 성공 지표 4단계
- **특징**: 무신사 MATCH 실행 가능 채널(앱푸시·이메일·디스플레이) 기준 구체화
- **실행**: `.claude/skills/gtm/SKILL.md` 워크플로우 따름
...
| **OKR 수립 · 검토** | **Skill: `/okr`** | claude-sonnet-4-6 |
...
**OKR, 목표 수립, Key Result, 분기 목표, KR 정의, 목표 검토, 분기 시작**
...
#### OKR 수립 · 검토 (`/okr`)
- **입력**: 분기 목표 텍스트 또는 `--align TM-XXXX` + 선택적 `--quarter Q1~Q4`
- **출력**: `output/okr/okr_{YYYYMMDD}_{팀}_{분기}.md`
- **모드**: `--draft`(초안 수립) / `--review {파일}`(기존 OKR 검토)
- **특징**: Jira 이니셔티브-KR 매핑, Amplitude/Databricks 측정 방법 제안, `/pgm` Flash 연계 형식 포함
- **방법론**: John Doerr Measure What Matters + Marty Cagan INSPIRED
- **실행**: `.claude/skills/okr/SKILL.md` 워크플로우 따름
...
| **시장 리서치 (페르소나·경쟁사·여정 지도)** | **Skill: `/market-research`** | claude-sonnet-4-6 |

### 트리거 키워드
Discovery, 시장 분석, 경쟁사, 화이트스페이스, 사용자 인터뷰, 벤치마크, 트렌드, Amplitude, Braze, Shopify, 외부 레퍼런스, 화면 분석, UI 분석, 화면 설계서, 서비스 분석,
**페르소나, ICP, 고객 여정, 경쟁사 포지셔닝, 포지셔닝 매트릭스, 타겟 고객, Jobs-to-be-Done, 시장 구조**
...
#### 시장 리서치 (`/market-research`)
- **입력**: 시장명 또는 주제 텍스트 + 선택적 옵션
  - `--focus persona|competitor|journey|all`: 분석 초점 (기본: all)
  - `--initiative TM-XXXX`: 이니셔티브 컨텍스트 로드
  - `--segment 글로벌|국내|B2C|B2B`: 타겟 세그먼트 힌트
- **출력**: `output/market-research/market_research_{YYYYMMDD}_{주제}.md`
- **구성**: 타겟 페르소나(Jobs-to-be-Done) + 경쟁사 포지셔닝 매트릭스 + 고객 여정 지도
- **차이점**: `/discovery`(기회 발굴 중심) vs `/market-research`(시장 구조 파악 중심) — 보완 관계
- **실행**: `.claude/skills/market-research/SKILL.md` 워크플로우 따름
...
## [지식팀] Confluence & Knowledge Base

### 역할
사내 지식 저장소(Confluence)에서 정보를 검색·조회하고, 새 문서를 생성·저장한다.
로컬 마크다운 기반 지식베이스(KB)를 구축·운영하여 에이전트 컨텍스트 품질을 높인다.
...
| **로컬 KB 저장·인덱싱·쿼리·익스포트** | **Skill: `/kb`** | claude-haiku-4-5 |

### 트리거 키워드
찾아줘, 알려줘, 어디에, 검색, Confluence, 저장해줘, 올려줘, 문서화, 위키,
**지식베이스, KB에 추가, KB 검색, KB 익스포트, NotebookLM 올릴 파일**
...
#### 로컬 KB (`/kb`)
- **입력**: 사용자가 붙여넣은 텍스트, 또는 `input/kb/` 내 마크다운 파일
- **출력**: `input/kb/{source}/YYYYMMDD_주제.md` + `input/kb/INDEX.md` + `output/kb_export_YYYYMMDD.md`
- **커맨드**: `/kb add` (저장) / `/kb index` (인덱싱) / `/kb query "질문"` (검색) / `/kb export` (NotebookLM용 익스포트)
- **privacy 필터**: `team` 레벨 파일은 export에서 자동 제외
- **다른 스킬 참조**: `/prd`, `/pgm`, `/analyst`, `/two-pager`가 initiative ID 또는 주제 기반으로 KB를 컨텍스트로 자동 로드
- **운영 가이드**: `input/kb/README.md`
...
# KB 저장 (Slack 대화 등 수동 큐레이션)
/kb add → input/kb/ 저장 → /kb index 갱신

# KB → NotebookLM 익스포트
/kb export → output/kb_export_YYYYMMDD.md → 사용자가 NotebookLM에 업로드
```
...
## [CRM 분석팀] CRM 전광판 지표 분석

### 역할
CRM 전광판 지표(발송 성과·세션 유입·수신 동의 모수·앱푸시 발송 현황)를 Databricks에서 자동 조회하고
트렌드·채널 비교·발송 압력·수신 동의 분석 및 이상 진단을 수행한다.

### 팀원 구성
| 역할 | 호출 방법 | 모델 |
|------|----------|------|
| CRM 분석 오케스트레이터 | Skill: `/crm-analysis` | claude-sonnet-4-6 |

### 트리거 키워드
CRM, 전광판, 발송 비용, 발송 수, 클릭 수, CTR, STR, 세션 유입, 리인게이지,
수신 동의, 앱푸시 동의, SMS 동의, 이메일 동의, 발송 압력, push_per_user,
CRM 성과, 채널 비교, CRM 지표, CRM 분석

### 지표 카테고리
| ID | 카테고리 | 핵심 지표 |
|----|---------|---------|
| A | CRM 발송 성과 | send_costs, send_count, click_count, CTR |
| B | 세션 유입 | count_sessions, STR |
| C | 수신 동의 모수 | total_user, 채널별 동의 유저, 동의율% |
| D | 앱푸시 발송 현황 | count_push_logs, count_push_user, push_per_user |
| Z | 통합 전광판 | A+B+C+D 전 지표 |

### 세부 역량

#### 전광판 스냅샷 (`/crm-analysis --snapshot`)
- **입력**: 없음 (어제 날짜 자동 적용)
- **출력**: `crm-analysis-agent-system/output/crm_analysis_{YYYYMMDD}_snapshot.md`

#### 기간 트렌드 분석 (`/crm-analysis --trend [7d/30d/90d]`)
- **입력**: 기간 (기본 30d)
- **출력**: `crm-analysis-agent-system/output/crm_analysis_{YYYYMMDD}_trend_{period}.md`

#### 채널별 성과 비교 (`/crm-analysis --channel`)
- **출력**: `crm-analysis-agent-system/output/crm_analysis_{YYYYMMDD}_channel.md`

#### 수신 동의 모수 분석 (`/crm-analysis --consent`)
- **출력**: `crm-analysis-agent-system/output/crm_analysis_{YYYYMMDD}_consent.md`

#### 앱푸시 발송 압력 분석 (`/crm-analysis --push`)
- **출력**: `crm-analysis-agent-system/output/crm_analysis_{YYYYMMDD}_push.md`

#### 지표 이상 원인 진단 (`/crm-analysis --diagnose "[지표]"`)
- **출력**: `crm-analysis-agent-system/output/crm_analysis_{YYYYMMDD}_diagnose_{지표}.md`

### 소스 테이블
- `team.marketing.musinsa_crm_campaign_list_daily`
- `team.marketing.metric_visit_purpose_sessions`
- `team.marketing.musinsa_crm_channel_cost`
- `team.marketing.metric_musinsa_marketing_ad_allow_user`
- `team.marketing.musinsa_app_push_send_logs`
...
### 역할
사용자가 입력한 데이터(CSV/JSON 등)를 분석 유형에 따라 자동 분기하여 인사이트 리포트를 생성하고,
Chart.js 기반 인터랙티브 HTML 차트로 시각화한다.
전처리·집계는 Haiku, 인사이트 도출·차트 생성은 Sonnet으로 역할을 분리한다.

### 팀원 구성
| 역할 | 호출 방법 | 모델 |
|------|----------|------|
| 분석 오케스트레이터 | Skill: `/analyst` | claude-haiku-4-5-20251001 |
| **차트 생성** | **Skill: `/chart`** | claude-sonnet-4-6 |
...
**차트 생성, 시각화, 그래프, TG/CG 비교 차트, CTR 트렌드 차트, 이벤트 마커, HTML 차트**
...
#### 차트 생성 (`/chart`)
- **입력**: CSV 데이터 파일 + x/y 컬럼 지정 + 선택적 스타일 파라미터
- **출력**: `output/charts/chart_{YYYYMMDD}_{주제}.html`
- **차트 유형**: `line` (기본) / `bar` / `ab-compare` (TG/CG 비교)
- **프리셋**:
  - `--preset ab-compare`: 파란 실선(TG) + 회색 점선(CG) + 이벤트 마커 (아래 디자인 스펙과 동일)
  - `--preset ctr-trend`: CTR 시계열 단일 라인 (% 포맷 자동 적용)
  - `--preset segment-bar`: 세그먼트 막대 비교 (Star/Growth/Risk/Dormant 색상)
- **이벤트 마커**: `--events "라벨:날짜:설명"` 반복 사용으로 수직 점선 + 원형 라벨 + 하단 범례 자동 생성
- **디자인 고정값**: Chart.js 4.4.1 / 높이 320px / 그리드 rgba(136,135,128,0.15) / 축 색상 #888780 / 이벤트 원 #EF9F27
- **실행**: `.claude/skills/chart/SKILL.md` 워크플로우 따름
...
# 분석 후 차트 시각화
→ /analyst --type ab-test --topic "Auxia PoC 결과"
→ (집계 CSV 저장 후)
→ /chart --preset ab-compare --data input/poc_ctr.csv --x date --y "tg_ctr,cg_ctr"
→ output/charts/ HTML 파일 생성
```

---

## [통합 데이터팀] 통합 데이터 조회 · 분석 · 시각화

### 역할

Databricks CRM 전광판 / Amplitude MCP / 파일 3가지 소스를 단일 파이프라인으로 연결하여
조회→분석→차트까지 한 번에 처리한다.
기존 `/crm-analysis`, `/analyst`, `/databricks` 스킬의 통합 진입점.

### 팀원 구성

| 역할 | 호출 방법 | 모델 |
|------|----------|------|
| 통합 데이터 오케스트레이터 | Skill: `/data` | claude-sonnet-4-6 |
| 소스 라우터 | Agent: `query-router` (data-agent-system 내) | claude-haiku-4-5-20251001 |
| CRM 조회 | Agent: `crm-querier` (data-agent-system 내) | claude-haiku-4-5-20251001 |
| Amplitude 조회 | Agent: `amplitude-querier` (data-agent-system 내) | claude-haiku-4-5-20251001 |
| Databricks 범용 조회 | Agent: `databricks-querier` (data-agent-system 내) | claude-haiku-4-5-20251001 |
| 데이터 전처리·집계 | Agent: `data-preprocessor` (data-agent-system 내) | claude-haiku-4-5-20251001 |
| 데이터 품질 검증 | Agent: `data-validator` (data-agent-system 내) | claude-haiku-4-5-20251001 |
| 시계열 트렌드 분석 | Agent: `trend-analyst` (data-agent-system 내) | claude-sonnet-4-6 |
| 세그먼트 비교 분석 | Agent: `segment-analyst` (data-agent-system 내) | claude-sonnet-4-6 |
| 퍼널 전환 분석 | Agent: `funnel-analyst` (data-agent-system 내) | claude-sonnet-4-6 |
| 코호트 리텐션 분석 | Agent: `cohort-analyst` (data-agent-system 내) | claude-sonnet-4-6 |
| A/B 테스트 평가 | Agent: `ab-test-analyst` (data-agent-system 내) | claude-sonnet-4-6 |
| Root Cause 진단 | Agent: `diagnose-analyst` (data-agent-system 내) | claude-sonnet-4-6 |
| 통합 인사이트 생성 | Agent: `insight-synthesizer` (data-agent-system 내) | claude-sonnet-4-6 |
| 차트 생성 | Agent: `chart-generator` (data-agent-system 내) | claude-sonnet-4-6 |

### 트리거 키워드

CRM 전광판, Amplitude, DAU, CTR, 트렌드, 퍼널, 코호트, 리텐션, A/B 테스트, 세그먼트,
지표 하락, Root Cause, 원인 분석, 차트, 시각화, 데이터 조회, 통합 분석

### 세부 역량

#### CRM 전광판 분석 (`/data --source crm`)
- 지원 모드: `--snapshot` / `--trend [7d/30d/90d]` / `--channel` / `--consent` / `--push` / `--diagnose "[지표]"`
- 이상 탐지 루프: CTR < 0.5% / STR < 1.0% / push_per_user > 5 / consent_rate WoW Δ −2%p↑
- 출력: `data-agent-system/output/data_crm_{YYYYMMDD}_{mode}.md`

#### Amplitude 분석 (`/data --source amplitude`)
- 지원 유형: trend / funnel / cohort / ab-test / segment / diagnose
- MCP 도구: `mcp__Amplitude__get_context`, `get_events`, `query_chart`, `get_experiments` 등
- 출력: `data-agent-system/output/data_{type}_{YYYYMMDD}_{주제}.md`

#### Databricks 범용 분석 (`/data --source databricks`)
- 자연어 → SQL 자동 변환 또는 SQL 직접 입력
- SELECT only 안전 규칙, 1000행 LIMIT 자동 적용
- 출력: `data-agent-system/output/data_{type}_{YYYYMMDD}_{주제}.md`
...
#### 차트 자동 생성 (`--chart` 플래그 또는 trend/ab-test 자동)
- Chart.js HTML 인터랙티브 차트
- 출력: `data-agent-system/output/charts/chart_{YYYYMMDD}_{주제}.html`

### 기존 스킬과의 관계
- `/crm-analysis` → `/data --source crm`과 동일 (기존 스킬도 계속 사용 가능)
- `/analyst` → `/data --source [amplitude|file]`과 동일
- `/databricks` → `/data --source databricks`와 동일
...
→ 이상 탐지 → 경보 시 자동 drill-down → 리포트 + 차트

# Amplitude 퍼널 분석
/data --source amplitude --type funnel --topic "홈→결제 퍼널" --chart
→ Amplitude MCP 조회 → 퍼널 분석 → 이탈 포인트 → 임팩트 시뮬레이션 → 차트

# 파일 기반 A/B 테스트
/data --file input/ab_result.csv --type ab-test --topic "앱푸시 실험" --chart
→ 전처리 → 통계 유의성 → 롤아웃 권고 → TG/CG 비교 차트

# 지표 이상 진단 (CRM)
/data --source crm --diagnose "CTR"
→ 최근 7일 vs 전 7일 → 가설 3개 → 검증 → Root Cause
```
...
## [커리어팀] Chronicle — 업무 결산 · 피드백 · 포트폴리오

### 역할
LLM Wiki 패턴으로 업무 이력을 누적 관리하고, 주간/월간 회고·피드백·플래닝·포트폴리오를 생성한다.

### 팀원 구성
| 역할 | 호출 방법 | 모델 |
|------|----------|------|
| 주간 회고 + 피드백 | Skill: `/chronicle weekly` | claude-sonnet-4-6 |
| 월간 결산 + 트렌드 | Skill: `/chronicle monthly` | claude-sonnet-4-6 |
| 다음 주/달 플래닝 | Skill: `/chronicle plan [week\|month]` | claude-sonnet-4-6 |
| 커리어 포트폴리오 추출 | Skill: `/chronicle portfolio` | claude-sonnet-4-6 |
| Wiki 수동 갱신 | Skill: `/chronicle ingest` | claude-sonnet-4-6 |
| Wiki 일관성 검사 | Skill: `/chronicle lint` | claude-sonnet-4-6 |

### 트리거 키워드
주간 결산, 월간 결산, 업무 회고, 이번 주 정리, 이번 달 정리, 피드백, 플래닝, 다음 주 계획,
포트폴리오, 이력서, 커리어 스토리, 성과 정리, 내가 한 일, wiki 갱신, 업무 기록

### 입력
- Staff session logs (`logs/staff_sessions.jsonl`)
- Jira (JQL: 내 티켓)
- Confluence (내가 수정한 페이지)
- `chronicle/input/guides/` (사용자 제공 피드백 기준: OKR, 우선순위, 성장 목표)
...
### Wiki 저장 위치
- `chronicle/input/wiki/` — LLM이 자동 유지보수하는 누적 지식 베이스
